...
class _HomeScreenState extends State<HomeScreen> {
  ...
  Widget build(BuildContext context) {
    final expenseRepo = SupabaseExpenseRepo();
    return MultiBlocProvider(
      providers: [
        BlocProvider(
          create: (context) => GetExpensesBloc(expenseRepo)..add(GetExpenses()),
        ),
        BlocProvider(
          create: (context) => DeleteExpenseBloc(expenseRepo),
        ),
      ],
      child: BlocBuilder<GetExpensesBloc, GetExpensesBlocState>(
        builder: (context, state) {
          if (state is GetExpensesSuccess) {
            return Scaffold(
                  ...
                  Navigator.push(
                    ...
                    MaterialPageRoute(
                      builder: (BuildContext context) => MultiBlocProvider(
                        providers: [
                          BlocProvider(
                            create: (context) => CreateExpenseBloc(expenseRepo),
                          ),
                          BlocProvider.value(
              ...
              body: index == 0
                  ? MainScreen(state.expenses, expenseRepo: expenseRepo)
                  : StatsScreen(),
            );
          } else {
            return Scaffold(
